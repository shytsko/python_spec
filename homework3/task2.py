...
for char in punctuation_chars:
    str_help = str_help.replace(char, "")

words = str_help.lower().split()

words_count = {}
for word in words:
    words_count[word] = words_count[word] + 1 if word in words_count else 1

most_frequent_words = sorted(words_count.keys(), key=words_count.get, reverse=True)[0:10]

print("Самые частые слова:")
for word in most_frequent_words:
    print(f"Слово '{word}' встречается {words_count[word]} раз(а)")
